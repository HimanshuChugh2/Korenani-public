...
public interface ClapsAnswerRepository extends JpaRepository<ClapsOfAnswer, Integer>  {
	...
	@Query(value = "update claps_of_answer set claps = claps+1 where aid= :aid" ,nativeQuery = true)
	@Transactional
	@Modifying
	public void updateClapsByAid(@Param("aid") Integer aid);
	
	@Query(value="select *from claps_of_answer where aid=:aid",nativeQuery = true)
	public List<ClapsOfAnswer> getByAid(@Param("aid") Integer aid);
	
	
	
	@Query(value = "update claps_of_answer set claps = claps+1, datetime= :datetime where aid= :aid and userprofilename= :userprofilename" ,nativeQuery = true)
	@Transactional
	@Modifying
	...
	@Query(value="select datetime from claps_of_answer where aid= :aid and userprofilename= :userprofilename",nativeQuery = true)
	...
	@Query(value="	select sum(claps) as clapcount, aid from claps_of_answer 	where aid in :listofaid group by aid",nativeQuery = true)
	public List<AnswerClapCountAndAidWrapper> getClapCountAidByAid(@Param("listofaid") List<Integer> aid);
	
	
	@Query(value="	select datetime,aid from claps_of_answer where aid in :listofaid and userprofilename = :userprofilename",nativeQuery = true)
