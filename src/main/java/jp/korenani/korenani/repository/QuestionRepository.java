...
public interface QuestionRepository extends JpaRepository<Question, Integer>{
	...
	@Query(value = "select * from korenani.question where id = :id", nativeQuery = true)
	public Question getById(@Param("id") int id);
	
	
	@Query(value="select title, date, id from korenani.question where userprofilename = :userprofilename ORDER BY date desc limit 2 ", nativeQuery = true)
	public List<QuestionPagintationWrapperInterface> getTwoQuestions(@Param("userprofilename") String userprofilename);
	

	@Query(value="select title, date, id from korenani.question where userprofilename = :userprofilename ORDER BY date desc limit 5 offset :offset", nativeQuery = true)
