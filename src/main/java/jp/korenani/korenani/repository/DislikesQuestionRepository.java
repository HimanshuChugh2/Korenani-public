...
public interface DislikesQuestionRepository extends JpaRepository<DislikesOfQuestion, Integer> {
	...
	@Query(value = "update dislikes_of_question set dislikes = dislikes+1, datetime= :datetime where qid= :qid and userprofilename= :userprofilename" ,nativeQuery = true)
	@Transactional
	@Modifying
	...
	@Query(value="select sum(dislikes) from dislikes_of_question where qid= :qid",nativeQuery = true)
	public Integer getDislikesCountByQid(@Param("qid") Integer qid);
	
	
	@Query(value="select datetime from dislikes_of_question where qid= :qid and userprofilename= :userprofilename",nativeQuery = true)
